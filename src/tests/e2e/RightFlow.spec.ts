...
test.describe("Right flow", () => {
  ...
  test("Create ticket", async ({ }) => {
    await page.locator('text=Vehicle typearrow_drop_down >> div').nth(1).click();
    await page.locator('div[role="listbox"] div:has-text("Car")').nth(3).click();
    await page.locator('[aria-label="Vehicle\\ plate"]').click();
    await page.locator('[aria-label="Vehicle\\ plate"]').fill('zzz000');
    await page.locator('button:has-text("Save")').click();
    await expect(page.locator('#dialogComponent')).toContainText('Ticket created!');
  });
  ...
  test("Closing ticket", async ({ }) => {
    ...
    } else if (await page.locator('button:has-text("chevron_right")').isVisible()) {
      await page.locator('button:has-text("chevron_right")').click();
    }

    const tbodyRowCount = await page.evaluate(() => ((document.getElementsByClassName("q-table")[0]) as HTMLTableElement).tBodies[0].rows.length);
    await page.locator('td:has-text("exit_to_app")').nth(tbodyRowCount - 1).click();
    await expect(page.locator('#dialogComponent')).toContainText('Ticket closed successfully');
  });
